...
public class MixinCactusBlock extends Block implements IGrowable
{
    ...
    public boolean canGrow(IBlockReader worldIn, BlockPos pos, BlockState state, boolean isClient) {
        ...
        if (upState.isAir(worldIn, upPos)) {
            ...
            return n + 1 < 3 && worldIn.getBlockState(pos).get(AGE) < 15;
        }
            ...
            return nUp + nDown + 1 < 3 & worldIn.getBlockState(pos.up(nUp)).get(AGE) < 15;
        }
        return false;
    }
    ...
    protected int getNumOfCactusBlocksAbove(IBlockReader worldIn, BlockPos pos) {
        int i = 0;
        while (i < 3 && worldIn.getBlockState(pos.up(i + 1)).getBlock() instanceof CactusBlock) i++;
        return i;
    }

    protected int getNumOfCactusBlocksBelow(IBlockReader worldIn, BlockPos pos) {
        int i = 0;
        while (i < 3 && worldIn.getBlockState(pos.down(i + 1)).getBlock() instanceof CactusBlock) i++;
        return i;
    }

    private void grow(ServerWorld worldIn, BlockPos pos, BlockState state, Random rand) {
        int age = Math.min(15, state.get(AGE) + rand.nextInt(3) + 2);
        if (age == 15) {
            BlockPos upPos = pos.up();
